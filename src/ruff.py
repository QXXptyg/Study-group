...
def setup_world(n_actors,client_mode):
    ...
    for i in range(n_actors):
        ...
        boxId = p.loadURDF("/home/oem/PycharmProjects/Ruff_ppo-main/URDF/SZpybullet.urdf",startPos, startOrientation)
...
class ruff:
    def __init__(self, id):
        ...
        self.delindex = [3,7,11,15,16,17]
        for self.INDEX in self.delindex:
           self.n_joints.remove(self.INDEX)
    ...
    def getjointinfo(self):
        ...
        print(n_joints)
        delindex = [3,7,11,15,16,17]
        for INDEX in delindex:
           n_joints.remove(INDEX)
        print(n_joints)
    ...
    def get_link_vel(self):
        self.foot_zvel = [p.getLinkState(self.id,3,1)[6][2],p.getLinkState(self.id,7,1)[6][2],p.getLinkState(self.id,11,1)[6][2],p.getLinkState(self.id,15,1)[6][2]]
        self.foot_xvel = [p.getLinkState(self.id,3,1)[6][0],p.getLinkState(self.id,7,1)[6][0],p.getLinkState(self.id,11,1)[6][0],p.getLinkState(self.id,15,1)[6][0]]
        self.foot_yvel = [p.getLinkState(self.id,3,1)[6][1],p.getLinkState(self.id,7,1)[6][1],p.getLinkState(self.id,11,1)[6][1],p.getLinkState(self.id,15,1)[6][1]]
    ...
    def get_state(self):
        ...
        state = state + list(i/(2*math.pi) for i in self.joint_position)+list(i/10 for i in self.joint_velocity)  ##  todo

        state = state + list([i/(2*math.pi) for i in self.pos_error])  ##todo
    ...
    def get_contact(self):
        self.is_contact = [p.getContactPoints(self.id,0,linkIndexA=3)!=(),p.getContactPoints(self.id,0,linkIndexA=7)!=(),
        p.getContactPoints(self.id,0,linkIndexA=11)!=(),p.getContactPoints(self.id,0,linkIndexA=15)!=()]
    def get_height(self):
        self.foot_height = [p.getClosestPoints(self.id,0,50000.0,linkIndexA=3)[0][8],p.getClosestPoints(self.id,0,50000.0,linkIndexA=7)[0][8],p.getClosestPoints(self.id,0,50000.0,linkIndexA=11)[0][8],p.getClosestPoints(self.id,0,50000.0,linkIndexA=15)[0][8]]
